  ...
  return (
    <main className="min-h-screen bg-[#080b12] text-gray-100">
      <header className="flex h-14 items-center justify-between border-b border-[#202838] bg-[#0c111b] px-3">
        ...
        <div className="flex items-center gap-1"><button className="p-2 text-gray-400" aria-label="Search"><Search size={17}/></button><button className="p-2 text-gray-400" aria-label="Notifications"><Bell size={17}/></button><button className="p-2 text-gray-400" aria-label="Settings"><Settings size={17}/></button></div>
      </header>
      <section className="grid min-h-[calc(100vh-56px)] grid-cols-1 md:grid-cols-[256px_minmax(0,1fr)_280px]">
        ...
        <div className="min-w-0">
          ...
          <div className="h-[60vh] min-h-[420px] md:h-[calc(100vh-185px)]"><TradingChart/></div>
          <div className="border-t border-[#202838] bg-[#0b1018]">
            ...
            {logOpen && <div className="px-3 pb-3 text-xs text-gray-500"><span className="text-emerald-400">ready</span> · chart synchronized · mock market data active · {orders.length} order(s)</div>}
          </div>
        </div>
        <aside className="hidden border-l border-[#202838] bg-[#0c111b] md:block">
          ...
          <div className="p-3">
            <div className="mb-3 grid grid-cols-2 rounded-lg bg-[#151c28] p-1">
              <button onClick={() => setSide("buy")} className={\`rounded py-2 text-sm font-semibold \${side === "buy" ? "bg-emerald-500/20 text-emerald-400" : "text-gray-500"}\`}>BUY</button>
              <button onClick={() => setSide("sell")} className={\`rounded py-2 text-sm font-semibold \${side === "sell" ? "bg-red-500/20 text-red-400" : "text-gray-500"}\`}>SELL</button>
            </div>
            <div className="mb-3 grid grid-cols-2 gap-1">
              <button onClick={() => setOrderType("market")} className={\`rounded border py-1.5 text-xs \${orderType === "market" ? "border-[#4b5c77] bg-[#1b2636]" : "border-[#263143] text-gray-500"}\`}>Market</button>
              <button onClick={() => setOrderType("limit")} className={\`rounded border py-1.5 text-xs \${orderType === "limit" ? "border-[#4b5c77] bg-[#1b2636]" : "border-[#263143] text-gray-500"}\`}>Limit</button>
            </div>
            ...
            <div className="mb-2 text-xs text-gray-500">Quantity</div><input value={quantity} min={1} onChange={e => setQuantity(Number(e.target.value))} type="number" className="mb-3 w-full rounded border border-[#263143] bg-[#101722] px-3 py-2 text-sm"/>
            {orderType === "limit" && <><div className="mb-2 text-xs text-gray-500">Limit Price</div><input value={limitPrice} onChange={e => setLimitPrice(e.target.value)} type="number" step="0.01" className="mb-3 w-full rounded border border-[#263143] bg-[#101722] px-3 py-2 text-sm"/></>}
            ...
            <button onClick={placeOrder} className={\`w-full rounded-lg py-2.5 text-sm font-bold text-black \${side === "buy" ? "bg-emerald-500" : "bg-red-400"}\`}><Wallet size={16} className="mr-2 inline"/>{side === "buy" ? "Buy" : "Sell"} {activeSymbol}</button>
          </div>
        </aside>
      </section>
      <footer className="border-t border-[#202838] bg-[#0c111b]"><div className="grid grid-cols-3 px-3 py-2 text-xs"><div><span className="text-gray-500">Equity</span><div className="font-semibold">\${account.equity.toLocaleString()}</div></div><div><span className="text-gray-500">Cash</span><div className="font-semibold">\${account.cash.toLocaleString()}</div></div><div><span className="text-gray-500">Buying Power</span><div className="font-semibold">\${account.buyingPower.toLocaleString()}</div></div></div></footer>
    </main>
  );
}